...
//    }
    inner class CommentRecyclerviewAdapter : RecyclerView.Adapter<CommentRecyclerviewAdapter.MessageViewHolder>() {
        ...
        init {
            fireDatabase.child("users").child(destinationUid.toString()).addListenerForSingleValueEvent(object : ValueEventListener{
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    friend = snapshot.getValue<FriendDTO>()
                    messageActivity_textView_topName.text = friend?.name
                    getMessageList()
                }
            })
        ...
        Log.d("클릭 시 time", comments[position].time.toString())
            ...
            if(comments[position].uid.equals(uid)){ // 본인 채팅
//                holder.textView_message.setBackgroundResource(R.drawable.rightbubble)
                holder.textView_message?.visibility = View.INVISIBLE
                holder.textView_name?.visibility = View.INVISIBLE
                holder.layout_main?.gravity = Gravity.RIGHT
            }else{ // 상대방 채팅
                Glide.with(holder.itemView.context)
